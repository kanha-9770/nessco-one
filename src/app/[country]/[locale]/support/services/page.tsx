...
import { ServicesItem } from "@/components/services/types/constant";
...
async function fetchservicesData(locale: string): Promise<ServicesItem | null> {
  try {
    const res = await fetch(`${apiUrl}${locale}/services.json`);
    const data = await res.json();
    return data;
  } catch (error) {
    const fallbackRes = await fetch(`${apiUrl}en/services.json`, {
...
export async function generateMetadata({
  ...
  const seoData = servicesData?.Services[0]?.servicesSeoData;
...
export default async function about({ params: { locale } }: Props) {
  ...
  return (
    <main>
      <Pages servicesData={servicesData} />
    </main>
  );
}
