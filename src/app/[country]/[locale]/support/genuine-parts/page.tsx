import Pages from "@/components/genuine-parts/Pages";
import { GenuinePartsItem } from "@/components/genuine-parts/types/constant";
...
async function fetchgenuinePartsData(
  locale: string
): Promise<GenuinePartsItem | null> {
  try {
    const res = await fetch(`${apiUrl}${locale}/genuineparts.json`);
    const data = await res.json();
    return data;
  } catch (error) {
    const fallbackRes = await fetch(`${apiUrl}en/genuineparts.json`, {
...
export async function generateMetadata({
  ...
  const genuinePartsData = await fetchgenuinePartsData(locale);

  if (!genuinePartsData && !countryName) {
    return {
      title: "Default Title",
  ...
  const seoData = genuinePartsData?.GenuineParts[0]?.genuinePartsSeoData;
...
export default async function about({ params: { locale } }: Props) {
  ...
  return (
    <main>
      <Pages genuinePartsData={genuinePartsData} />
    </main>
  );
}
