import Pages from "@/components/videos/Header";
import { VideosItem } from "@/components/videos/types/constant";
...
async function fetchvideosData(locale: string): Promise<VideosItem | null> {
  try {
    const res = await fetch(`${apiUrl}${locale}/videos.json`);
    const data = await res.json();
    return data;
  } catch (error) {
    const fallbackRes = await fetch(`${apiUrl}en/videos.json`, {
...
export async function generateMetadata({
  ...
  const videosData = await fetchvideosData(locale);

  if (!videosData && !countryName) {
    return {
      title: "Default Title",
  ...
  const seoData = videosData?.Videos[0]?.videosSeoData;
