...
const bgColors = ["bg-[#f4f4ff]", "bg-[#f6ffef]", "bg-blue-200", "bg-yellow-200"];
...
const AboutLayout: React.FC<AboutDataProps> = ({ navData, setActive }) => {
  ...
  const scrollDown = useCallback(() => {
    setCurrentIndex((prev) => Math.min(prev + 1, navLeftData.length - 2));
  }, [navLeftData.length]);
  ...
  const renderNavItem = (item: NavItem, index: number) => (
    ...
    >
      <div className="flex flex-row lg:flex-col space-x-4 lg:space-x-0">
        <Image
          src={item.image || "https://assets.nesscoindustries.com/public/assets/about/about-main-icon.webp"}
  ...
  const renderLeftNavItem = (item: NavItem, index: number) => (
    <Link
      key={index}
      href={`/${countryCODE}/${languageCODE}/about/${item.link}`}
      onClick={() => setActive?.(null)}
    ...
    >
      <div
        className={`hidden lg:flex h-[6.5rem] lg:hover:scale-80 transition-transform duration-200 items-center lg:p-4 lg:rounded-3xl ${
          bgColors[index % bgColors.length]
        }`}
      >
        <div className={`w-[20%] mx-4 flex justify-center items-center text-2xl ${item.textcolor}`}>
          <Image
        ...
        <div className="w-[80%]">
          <h3 className="text-sm sm:text-md text-black font-semibold mb-0">{item.title}</h3>
          <p className="text-xs hidden lg:flex font-regular text-black pr-2">{item.description}</p>
        </div>
      </div>
      <div
  ...
  return (
    <div className="flex w-full lg:border-none lg:pb-6 mx-auto max-w-screen-2xl flex-col lg:flex-row items-center justify-center lg:rounded-xl h-full">
      <div className="grid grid-cols-1 gap-2 mt-2 px-4 h-full sm:grid-cols-3 lg:grid-cols-4 w-full lg:w-[70.5vw]">
        ...
        {navRightData?.map(renderNavItem)}
      </div>
      <div className="lg:flex lg:px-0 px-4 ml-2 w-2 h-72 hidden border-l border-gray-300"></div>
      <div className="w-full lg:w-[20vw] h-32 ml-4 lg:h-auto flex flex-col justify-between mt-4 lg:mt-0" ref={carouselRef}>
        {navLeftData.slice(currentIndex, currentIndex + 2).map(renderLeftNavItem)}
